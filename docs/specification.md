...
| キー | 内容 | 既定値 |
| --- | --- | --- |
| `shape` | `"box"` / `"sphere"` / `"cylinder"` | `"box"` |
| `width` / `height` / `depth` | 直方体の全長（中心からではない） | 各 1 |
| `radius` | 球・円柱の半径 | 0.5 |
| `height` | 円柱の高さ（直方体と共通） | 1 |
| `offsetX` / `offsetY` / `offsetZ` | オブジェクトの座標からのずれ | 各 0 |

**円柱は Y軸に沿って立つ**形に固定する（見た目の `cylinder` と同じ向き）。
横倒しの円柱は扱わない。

- 知らない `shape` は直方体として扱う
...
| `shape: "sphere"` | **球**（3方向の輪。2Dなら正面の輪1つ） |
| `shape: "cylinder"` | **円柱**（上下の輪と縦4本。2Dなら矩形） |
| 判定を持たないもの | 描かない |
...
- **並べ替えるのはオブジェクト単位**。1つのモデルの中の面どうしは並べ替えない
  - 裏面を隠していないため、半透明の箱は**手前の面ごしに奥の面も見える**
  - ガラスとしては自然だが、面の描く順はモデルの並び順に従うため、複雑な形では濃さが不揃いになることがある
  - 裏面を隠す指定（面の間引き）は、必要になった時点で検討する
...
- 面の間引きをしていないため、**箱の上面と底面の2枚を通る**
- `@ALPHA = 0.5` の箱なら、影の透過率は `0.5 × 0.5 = 0.25` になる
- 中が空のガラス箱を通る光としては自然だが、**見た目の濃さとは一致しない**
